...
void gspmmv(
    ...
    cudaMemcpyAsync(d_offsets, h_offsets, sizeof(int) * num_offsets, cudaMemcpyHostToDevice, stream);
    cudaMemcpyAsync(d_neighbors, h_neighbors, sizeof(int) * ecount, cudaMemcpyHostToDevice, stream);
    ...
    spmmv_kernel<<<gridSize, blockSize, 0, stream>>>(
        vcount,
        feature_dim,
        d_offsets,
        d_neighbors,
